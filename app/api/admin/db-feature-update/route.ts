...
export async function GET(req: NextRequest) {
    ...
    try {
        await db.prepare('ALTER TABLE products ADD COLUMN updated_at DATETIME').run()
        results.push('Added updated_at to products')
    } catch (e: any) { results.push(`updated_at: ${e.message}`) }

    try {
        await db.prepare('ALTER TABLE products ADD COLUMN is_active INTEGER DEFAULT 1').run()
        results.push('Added is_active to products')
    } catch (e: any) { results.push(`is_active: ${e.message}`) }
